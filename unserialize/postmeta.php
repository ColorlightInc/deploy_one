...
$result = $conn->query("SELECT COUNT(*) FROM wp_postmeta");
$rows = $result->fetch_row();
$batches = $rows[0] / 2500;
$smt = $conn -> prepare('UPDATE wp_postmeta SET meta_value=? WHERE meta_id=?');

for ($i = 0; $i <= $batches; $i++) {
	$offset = $i * 2500;
	$result = $conn->query("SELECT * FROM wp_postmeta LIMIT 2500 OFFSET ".$offset);
